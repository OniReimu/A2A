...
async def setup_server(host, port):
    ...
    try:
        ...
        logger.info("Running agent warm-up commands...")
        try:
            # Set a maximum timeout for all initialization commands
            await asyncio.wait_for(
                agent.run_initialization_commands(),
                timeout=60  # 60 seconds total timeout for all initialization
            )
            logger.info("Initialization commands completed successfully")
        except asyncio.TimeoutError:
            logger.warning("Initialization commands timed out, but continuing with server startup")
        except Exception as e:
            logger.warning(f"Initialization commands failed, but continuing: {e}")
